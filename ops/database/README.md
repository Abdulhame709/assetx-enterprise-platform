...
يجب أن تكون الترحيلات الحالية ثمانية، وأن يظهر سياق المستأجر عند تشغيل العملية، وأن تعيد RLS بيانات المستأجر المطلوب فقط. يجب أن يفشل الطلب المحمي بلا JWT، وأن ينجح مع صلاحية صحيحة، وأن يعيد `/health` حالة قاعدة البيانات `ok`. بعد ذلك يُختبر `pg_dump` و`pg_restore` إلى قاعدة منفصلة، ثم PITR وRPO/RTO على المزود السحابي قبل اعتماد production.
