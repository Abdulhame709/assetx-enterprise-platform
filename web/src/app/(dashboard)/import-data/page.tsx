...
import { DATA_IMPORT_RESOURCES, DataImportPreview, DataImportResource, downloadDataImportTemplate, executeDataImport, isDataImportResource, previewDataImport } from '@/features/imports/asset-import-api';
import { IMPORT_RESOURCE_COLUMNS, IMPORT_RESOURCE_LABELS } from '@/features/imports/import-resource';
...
export default function ImportDataPage() {
  ...
  const [resource, setResource] = useState<DataImportResource>(() => isDataImportResource(searchParams.get('resource')) ? searchParams.get('resource') as DataImportResource : 'assets');
  ...
  useEffect(() => {
    const fromQuery = searchParams.get('resource');
    if (isDataImportResource(fromQuery)) { setResource(fromQuery); setFile(null); setPreview(null); if (inputRef.current) inputRef.current.value = ''; }
  }, [searchParams]);
  ...
  const selectFile = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try { setPreview(await previewDataImport(resource, selected)); }
  ...
  const downloadTemplate = async () => {
    setDownloading(true);
    try { await downloadDataImportTemplate(resource); toast.success(t('module.importDownloaded'), t(IMPORT_RESOURCE_LABELS[resource])); }
  ...
  const execute = async () => {
    ...
    setExecuting(true);
    try {
      const result = await executeDataImport(resource, file);
      setPreview(result);
      toast.success(t('importData.completeTitle'), `${(result.imported ?? 0).toLocaleString(locale)} ${t('importData.recordsImported')}`);
  ...
  const changeResource = (next: DataImportResource) => { setResource(next); reset(); };
  const columns = IMPORT_RESOURCE_COLUMNS[resource];

  return <div className="space-y-4">
    <PageHeader title={t('nav.importData')} subtitle={t('importData.pageSubtitle')} actions={<Button variant="secondary" size="sm" loading={downloading} onClick={() => void downloadTemplate()}><Download className="h-4 w-4" /> {t('importData.downloadTemplate')}</Button>} />
    <section className="rounded-xl border border-line bg-surface-raised p-3 shadow-card" aria-label={t('importData.resource')}><p className="mb-2 text-xs font-semibold text-ink-muted">{t('importData.resource')}</p><div className="flex flex-wrap gap-2">{DATA_IMPORT_RESOURCES.map((item) => <Button key={item} variant={resource === item ? 'primary' : 'secondary'} size="sm" onClick={() => changeResource(item)}>{t(IMPORT_RESOURCE_LABELS[item])}</Button>)}</div><p className="mt-2 text-xs text-ink-muted">{resource === 'assets' ? t('importData.assetsHint') : t('importData.masterDataHint')}</p></section>
    <section className="grid gap-4 xl:grid-cols-[1.05fr_1.95fr]">
      ...
      <Card className="overflow-hidden shadow-card"><CardHeader title={t('importData.previewTitle')} subtitle={t('importData.previewSubtitle')} /><CardBody>
        ...
        {!loadingPreview && preview && <div className="space-y-4">
          ...
          {preview.rows.length > 0 && <div className="overflow-x-auto rounded-xl border border-line"><table className="min-w-full text-sm"><thead className="bg-surface-muted text-xs text-ink-muted"><tr><th className="px-3 py-2 text-start">#</th>{columns.map((column) => <th key={column.key} className="px-3 py-2 text-start">{t(column.labelKey)}</th>)}</tr></thead><tbody className="divide-y divide-line">{preview.rows.map((row) => <tr key={row.row}><td className="px-3 py-2 font-mono text-xs text-ink-faint">{row.row}</td>{columns.map((column) => <td key={column.key} className="px-3 py-2 text-ink-muted">{row.values[column.key] ?? '—'}</td>)}</tr>)}</tbody></table></div>}
